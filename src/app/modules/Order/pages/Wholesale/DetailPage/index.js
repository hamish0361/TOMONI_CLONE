...
function WholesaleDetailPage({
    ...
    const [paramItems, setParamItems] = useState({
        ...
        with: 'orderItemPurchases.purchase',
        search: `order_id:${id}`
    });
    ...
    const paramDetail = {
        ...
        params: {
            with:
                'shipmentInfor;items.orderItemPurchases.purchaseItem.tracking',
    ...
    const listTracking = orderDetail?.items?.map(item =>
        item?.order_item_purchases?.map(x => x?.purchase_item?.tracking)
    );
    const rowsTracking = listTracking?.[0]?.map(tracking => {
        return {
            id: tracking?.id || '-',
            checked: (
                <span
